...
def fetch_page(url: str, proxy: Optional[str] = None, user_agent: str = None) -> str:
    """
    Fetch the HTML content of a page with optional proxy and user-agent settings.
    """
    headers = {
...
def scrape_products(base_url: str, page_limit: int = 5, proxy: Optional[str] = None, rate_limit_seconds: int = 1) -> List[Dict[str, str]]:
    ...
    for page_number in range(1, page_limit + 1):
        ...
        time.sleep(rate_limit_seconds)  # Respect server load
    return products
